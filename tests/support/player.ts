...
import type { Run, Unit, Skill, Difficulty } from "../../src/types";
...
function score(r: Run, h: Unit, s: Skill, t: Unit, value: number) {
  ...
  switch (s.effect) {
    ...
    case "rewind":
      return b.dice.filter(d=>d.used).length >= 4 ? 100 : 0;
    default:
      return 0;
  }
}
export function fight(r: Run) {
  let actions = 0;
  while (r.screen === "battle" && actions++ < 450) {
    let best: {
      h: Unit;
      s: Skill;
      id: number | null;
  ...
  expect(actions).toBeLessThan(451);
}
export function sim(seed: string, starter: string, difficulty: Difficulty = "normal") {
  const r = G.createRun(seed, starter, difficulty, {version:1,hardCleared:true,paradoxCleared:false});
  let steps = 0;
  while (!["won", "lost"].includes(r.screen) && steps++ < 450) {
    switch (r.screen) {
      case "rewind":
        G.rewindCycle(r);
        break;
      ...
      case "rest":
        G.rest(
          r,
          !G.canUpgrade(r) || r.party.some((h) => h.hp < h.maxHp * 0.75) ? "heal" : "upgrade",
        );
        break;
      case "upgrade": {
        const h = [...r.party].filter(h=>MODS.some(m=>h.mods.filter(x=>x===m.id).length<3)).sort((a, b) => a.level - b.level)[0];
        if(!h){G.continueToMap(r);break;}
